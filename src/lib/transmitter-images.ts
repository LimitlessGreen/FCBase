...
const transmitterImageMap = new Map<string, ImageMetadata>(
  Object.entries(localTransmitterImages).map(([path, module]) => {
    const file = path.split('/').pop();
    const id = file?.replace(IMAGE_EXTENSION_PATTERN, '');
    if (!id) {
      throw new Error(`Unable to derive transmitter image id from path: ${path}`);
    }
...
const findLocalImage = (candidates: Iterable<string | undefined>) => {
  for (const candidate of candidates) {
    ...
    for (const variant of toKeyVariants(candidate)) {
      const match = transmitterImageMap.get(variant.toLowerCase());
      if (match) {
        return match;
      }
    }
...
export type TransmitterPreviewImage = {
...
export const resolveTransmitterPreviewImage = (
  ...
  const heroImage = transmitter.data.images?.[0];

  const alt = heroImage?.alt ?? transmitter.data.title;
  ...
  const slug = (transmitter as { slug?: string }).slug;

  const candidates = new Set<string | undefined>();
  candidates.add(transmitter.id);
  candidates.add(slug);
  candidates.add(transmitter.data.id);
  if (transmitter.id.includes('/')) {
    candidates.add(transmitter.id.split('/').pop());
  }
  if (slug?.includes('/')) {
